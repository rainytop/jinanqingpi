function myFormSubmit(url) {
    $('#myForm').ajaxSubmit(function () {
        $('#loading').show();
        $('#loading div').html('更新成功!');
        openUrl(url);
    });
}
...
function myFormSubmit2(url) {
    $('#myForm2').ajaxSubmit(function () {
        $('#loading').show();
        $('#loading div').html('更新成功!');
        openUrl(url);
    });
...
function myFormSearchSubmit() {
    $("#loading-loader").show();
    $('.nav li a').removeClass('current');
    $('#myFormSearch').ajaxSubmit(function (data) {
...
function openUrl(url, o) {
    ...
    if (o) {
        $('.nav li a').removeClass('current');
        if ($(o).parent().parent().hasClass('nav')) {
            $(o).addClass('current');
        } else {
            $(o).parent().parent().prev().addClass('current');
        }
    }
}

function openUrlReturn(url, returnUrl) {
    $.ajax({
        type: "post",
        url: url,
        data: {},
...
var themeprimary = getThemeColorFromCss("themeprimary"), themesecondary = getThemeColorFromCss("themesecondary"), themethirdcolor = getThemeColorFromCss("themethirdcolor"), themefourthcolor = getThemeColorFromCss("themefourthcolor"), themefifthcolor = getThemeColorFromCss("themefifthcolor"), rtlchanger, loadings, hoverloadings;
...
loadings = $("[data-toggle=loading]");
$.each(loadings, function () {
    $(this).loading({
        html: !0,
        template: '<div class="loading ' + $(this).data("class") + '"><div class="arrow"><\/div><h3 class="loading-title ' + $(this).data("titleclass") + '">loading right<\/h3><div class="loading-content"><\/div><\/div>'
    })
});
hoverloadings = $("[data-toggle=loading-hover]");
$.each(hoverloadings, function () {
    $(this).loading({
        html: !0,
        template: '<div class="loading ' + $(this).data("class") + '"><div class="arrow"><\/div><h3 class="loading-title ' + $(this).data("titleclass") + '">loading right<\/h3><div class="loading-content"><\/div><\/div>',
        trigger: "hover"
